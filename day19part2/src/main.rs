...
fn main() {
    ...
    let scanners_pos = scanners_abs_pos(&mut scanners);
    let mut largest_manhattan_dist = 0;

    for i in 0..scanners_pos.len()-1 {
        for j in i..scanners_pos.len() {
            let s1 = scanners_pos[i];
            let s2 = scanners_pos[j];
            let manhattan_dist =
                (s1.0 - s2.0).abs() +
                (s1.1 - s2.1).abs() +
                (s1.2 - s2.2).abs();
            largest_manhattan_dist = largest_manhattan_dist.max(manhattan_dist);
        }
    }



    println!("{}", largest_manhattan_dist);
}
