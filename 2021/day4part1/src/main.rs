...
fn main() {
    ...
    let mut least_moves = -1;
    let mut best_board: i32 = -1;
    let mut last_called = -1;
    let mut best_board_marked: Vec<Vec<i32>> = vec![vec![0; 5]; 5];
    let mut board_idx = 0;
    ...
    for board in &boards {
        ...
        for draw in &draws {
            ...
            if board_won(&board_marked) {
                if best_board == -1 || num_moves < least_moves {
                    best_board = board_idx;
                    least_moves = num_moves;
                    best_board_marked = board_marked;
    ...
    for row in 0..5 {
        for col in 0..5 {
            if best_board_marked[row][col] == 0 {
                sum += boards[best_board as usize][row][col]
                    .parse::<i32>()
                    .unwrap();
            }
        }
